...
fn field(q: vec2f, t: f32) -> f32 {
  ...
  var d = rounded_box(q - travel(t, 0.0, 8.0, 0.66), half_size, 0.055);
  var previous = travel(t, 0.0, 8.0, 0.66);
  for (var i = 1; i < LINKS; i = i + 1) {
    let f = f32(i);
    let at = travel(t, f * 0.030, 8.0, 0.66);
    let shrink = 1.0 - f / f32(LINKS) * 0.55;
    let link = rounded_box(q - at, half_size * shrink, 0.05 * shrink);
    let gap = length(at - previous);
    d = smin(d, link, 0.030 + gap * 1.4);
    previous = at;
  }
  return d;
}
